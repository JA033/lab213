...
__inline int select_in_word_little(const uint64_t x, const int k) {
    ...
    byte_sums = (byte_sums & 3ULL * ONES_STEP_4) +
                ((byte_sums >> 2) & 3ULL * ONES_STEP_4);
    ...
    const int place = popcnt(EASY_LEQ_STEP_8_MSBS(byte_sums, k_step_8)) * 8;
    // Phase 3: Locate the relevant byte and look up the result in
    // select_in_byte
    return place +
           select_in_byte[(x >> place & 0xFFULL) |
                          (k - ((byte_sums << 8) >> place & 0xFFULL)) << 8];
}

__inline int select_in_word_big(const uint64_t x, const int k) {
    const int k_new = popcnt(x) - k;
